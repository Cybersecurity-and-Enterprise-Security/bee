...
1. If you do not want to forward specific ports, e.g. if you need **SSH access** to the bee via its bound ip, use the `-ignoredTcpPorts` and `-ignoredUdpPorts` variables. These are comma-separated lists of port ranges, where each range is only one port or `start-end`, e.g. `1-1023,2222`.
